...
// 红色hsv值
int minH = 0, maxH = 190;
int minS = 55, maxS = 106;
int minV = 135, maxV = 194;
// 滤波器
class Filter
{
public:
...
vector<Vec4i> lines_process(Mat src)
{
    ...
    vector<Vec4i> lines;
    HoughLinesP(open, lines, 1, CV_PI / 180, 80, 110, 10);

    if (lines.size() > 0)
    {
        lines = filterLines(lines, 40);
        lines = filterLines_depth(lines, 2900);
        for (size_t i = 0; i < lines.size(); i++)
        {
            Vec4i l = lines[i];
...
double get_circle_depth(Mat src)
{
    double erro_circle;
    int max_depth = 0;
    Mat structure_close = getStructuringElement(MORPH_RECT, Size(9, 9));
    Mat close;

    // 闭运算，连接特征点
    morphologyEx(src, close, MORPH_DILATE, structure_close);
    // morphologyEx(close, close, MORPH_CLOSE, structure_close);
    // imshow("close", close);

    vector<std::pair<int, int>> leftPoints, rightPoints;
    int midY = depth_image.rows / 2;
    ...
    for (int y = midY - 10; y < midY + 10; y++)
    {
        for (int x = 0; x < depth_image.cols; ++x)
        {
            ...
            int color_value = close.at<uchar>(y, x);

            // if (depthValue > max_depth)
            // {
            //     max_depth = depthValue;
            // }
            if (depthValue > 50 && depthValue < 4000 && color_value)
            {
...
// 订阅状态变量
void doOpencv_sub(const detection::detection::ConstPtr &msg)
{
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher detection_pub = nh.advertise<detection::detection>("detection_stage1", 1);
    ros::Publisher detection_pub_stage2 = nh.advertise<detection::detection>("detection_stage2", 1);
    ros::Subscriber Opencv_sub = nh.subscribe<detection::detection>("detection_status", 10, doOpencv_sub);
    ...
    pid_controller_position.setParameters(0.3, 0.0, 0.1, -300.0, 300.0);
    ...
    while (ros::ok())
    {
        ...
        if (status == 2)
        {
            ...
            if (lines.size())
            {
                ...
                dz = pid_controller_depth.update(1200, depth); // 保持1.6m距离
                filter_depth.update(dz);
        ...
        cv::line(color_image, cv::Point(0, centerY), cv::Point(imageWidth, centerY), cv::Scalar(0, 0, 255), 2);
