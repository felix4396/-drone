...
void doOpencv_sub(const drone_test::detection::ConstPtr &msg)
{
    ...
    opencv_erro_x = opencv_erro_x / 100.0 * max_vel_x; // 相当于归一化
    opencv_erro_y = opencv_erro_y / 100.0 * max_vel_y;
...
int main(int argc, char **argv)
{
    ...
    /*----------------------------------------------------------------------------------------------------------------------------------- */
    ros::Subscriber sub = nh.subscribe<drone_test::Y_Serial_port>("serial_chatter", 100, doPerson);
...
CHECK:
    ...
    else
    {
        ...
        /*---------------------------------------------------------第一阶段--------------------------------------------------------------------- */
        ROS_INFO("Start First Stage");
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 2; j++)
            {
                if (fly_tar[j] == first_lev[i])
                {
                    fly_state = first_lev[i];
                    if (fly_state)
                    {
                        ROS_INFO("1");
                        yopcv.start_opencv += 1;

                        if (yopcv.start_opencv == 1)
                        {
                            yopcv3.flag = fly_state;
                        }
                        else if (yopcv.start_opencv == 2)
                        {
                            yopcv3.start_opencv = fly_state;
                        }

                        ros::spinOnce();
                        set_pose_body(x_pos[fly_state - 1] - now_position[0], y_pos[fly_state - 1] - now_position[1], 0.0, 0.0);
                        ros::Duration(0.5).sleep();
                        do
                        {
                            detection_pub.publish(yopcv);
                            array_pub.publish(yopcv3);
                            rate.sleep();
                            ros::spinOnce();
                        } while (ros::ok() && Pos_mode != Position_ControlMode_Position);
                        ros::spinOnce();
        ...
        /*---------------------------------------------------------第二阶段--------------------------------------------------------------------- */
        ROS_INFO("Start Second Stage");
        for (int i = 0; i < 5; i++)
        {
            for (int j = 0; j < 2; j++)
            {
                if (fly_tar[j] == sec_lev[i])
                {
                    fly_state = sec_lev[i];
                    if (fly_state)
                    {
        ...
        /*---------------------------------------------------------第三阶段--------------------------------------------------------------------- */
        ROS_INFO("Start Third Stage");
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 2; j++)
            {
                if (fly_tar[j] == third_lev[i])
                {
                    fly_state = third_lev[i];
                    if (fly_state)
                    {
                        ROS_INFO("3");
                        yopcv.start_opencv += 1;
        ...
        /*---------------------------------------------------------第四阶段--------------------------------------------------------------------- */
        ROS_INFO("Start Fourth Stage");
        for (int i = 0; i < 1; i++)
        {
            for (int j = 0; j < 2; j++)
            {
                if (fly_tar[j] == forth_lev[i])
                {
                    fly_state = forth_lev[i];
                    if (fly_state)
                    {
                        ROS_INFO("4");
                        yopcv.start_opencv += 1;
    ...
    yopcv.start_opencv += 1;
    ros::spinOnce();
    set_pose_body(takeoff_x - now_position[0] - 0.1, takeoff_y - now_position[1] + 0.1, 0.0, 0.0);
    ros::Duration(0.5).sleep();
    do
    {
        detection_pub.publish(yopcv);
